...
# - 아두이노에서 "2\n" → 일시정지 ON
# - 아두이노에서 "3\n" → 일시정지 OFF
# - 카메라를 항상 켜두고(저속 펌프), 트리거 시 버스트로 최신 프레임 확보
# - ★ 라벨 16자리의 마지막(s[15]) = wrap_layers(감싸는 횟수)
# - ★ 방향 최적화: 롤폭(기본 200mm)으로 덮는 축을 선택, 줄수=bands=ceil(덮는축/롤폭)
# - DM 없으면 모델 라벨(16자리)로 타입/치수 파싱 → UI 반영
# - p < 0.40 → "오류발생(빨간)" / p ≥ 0.40 → "분석 완료(초록)"
...
def parse_dm_label_16(s):
    """
    [0:6]=type_id, [6:9]=W, [9:12]=L, [12:15]=H, [15]=wrap_layers(감싸는 횟수)
    """
    if s is None:
        return None
    s = str(s).strip()
    ...
    try:
        return {
            ...
            "L":           int(s[9:12]),
            "H":           int(s[12:15]),
            "wrap_layers": int(s[15]),   # ★ 마지막 자리를 감싸는 횟수로 사용
            "raw":         s,
        }
    except Exception:
        return None

# ---- ★ 라벨용 계산: overlap/slack 없이 방향 최적화 (둘레×회전수×줄수) ----
...
def _set_ui_from_label(s):
    ...
    # ★ 라벨일 때: overlap/slack 없이 방향 최적화 계산
            ...
            if trigger_scan:
                ...
                try:
                    ...
                    if low_prob:
                        ...
                        _set_status("error")   # p<th → 오류(빨간)
                    else:
                        _set_warn(None)
                        _set_status("done")    # p>=th → 완료(초록)
